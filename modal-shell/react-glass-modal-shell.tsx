  ...
  return (
    <div className="fixed inset-0 z-[250] flex items-center justify-center p-2 sm:p-4" role="dialog" aria-modal="true">
      <button
        ...
        className={cn('fixed inset-0 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300', overlayClassName)}
        onClick={onClose}
      />

      <div
        className={cn(
          'bg-zinc-900 border border-white/10 w-full rounded-[2.5rem] shadow-2xl overflow-hidden relative z-10 animate-in zoom-in-95 duration-300 flex flex-col min-h-0 max-h-[calc(100dvh-1rem)] sm:max-h-[calc(100dvh-2rem)]',
          maxWidthClassName,
          panelClassName,
        )}
      >
        <div className="px-4 py-4 sm:p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02] shrink-0 gap-3 sm:gap-4">
          <div className="flex items-center gap-4 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="text-xl font-black text-white tracking-tight truncate">{title}</h3>
              {subtitle ? (
                <p className="text-xs sm:text-sm font-bold opacity-40 tracking-widest mt-1 truncate">{subtitle}</p>
              ) : null}
            </div>
          </div>
          {closeButton ?? (
        ...
        <div className={cn('flex-1 min-h-0 overflow-y-auto overscroll-contain custom-scrollbar p-4 sm:p-6 lg:p-8', bodyClassName)}>
          {children}
        </div>

        {footer ? (
          <div className="px-4 py-4 sm:p-6 lg:p-8 bg-white/[0.02] border-t border-white/5 shrink-0">
            {footer}
          </div>
        ) : null}
      </div>
