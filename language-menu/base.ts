...
export type StaticLocaleMenuItem = {
  readonly code: SupportedLocale;
  readonly label: string;
  readonly pathPrefix: string;
...
export function createLocaleMenuControlStyle(): string {
  return [
    ...
    `--fu-control-menu-width:${LOCALE_MENU_METRICS.menuWidthPx}px`,
  ].join(';');
}

export function resolveCurrentPathLocale<TOption extends { code: SupportedLocale; pathPrefix: string }>(
  pathname: string,
  localeOptions: readonly TOption[],
): TOption {
  const fallbackLocale = localeOptions[0];
  if (!fallbackLocale) {
    throw new Error('localeOptions must not be empty.');
  }
      ...
      option.pathPrefix
        ...
        : !localeOptions.some(
            (candidate) =>
              ...
              (pathname === candidate.pathPrefix || pathname.startsWith(`${candidate.pathPrefix}/`)),
          ),
    ) ?? fallbackLocale
  );
}

export function createStaticLocaleMenuItems<TOption extends { code: SupportedLocale; label: string; pathPrefix: string }>(
  localeOptions: readonly TOption[],
  currentLocale: { code: SupportedLocale },
): readonly StaticLocaleMenuItem[] {
  return localeOptions.map((option) => ({
    code: option.code,
    label: option.label,
    pathPrefix: option.pathPrefix,
    selected: option.code === currentLocale.code,
  }));
}
